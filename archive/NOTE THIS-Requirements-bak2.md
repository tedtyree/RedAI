...
- The text after the cursor becomes a new block of type `p` (paragraph), except:
  - If the original block was a `ul` or `ol` type (any indent level), the new block inherits the same type and level (see §11.6 for list-specific rules including auto-increment for `ol`).
  ...
  - If the original block is a `p` block whose trimmed text is exactly `---`, the block converts to `hr` and a new empty `p` block is created after it (see §7.6).
...
| `ul1`–`ul5` | Indented bullet items; prefix is `- ` with 2 spaces per level of indentation (level 1 = no indent). See §11.7 for per-level detail. Consecutive list blocks are grouped without blank lines between them. One blank line after the last item in the group. |
| `ol1`–`ol5` | Indented numbered items; text includes the user-managed numeric prefix (e.g. `1. `). Indented with 2 spaces per level (level 1 = no indent). See §11.7 for per-level detail. Same grouping rules as `ul`. |
